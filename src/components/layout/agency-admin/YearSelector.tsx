...
export default function YearSelector({
  ...
  return (
    <div className="relative">
      <button
        onClick={() => setOpen((v) => !v)}
        className="flex items-center gap-2 px-3 py-1.5 bg-primary/8 hover:bg-primary/15 text-primary rounded-lg text-sm font-semibold transition-colors border border-primary/20"
      >
            ...
            {availableYears.map((y) => (
              ...
              >
                ...
                {y === new Date().getFullYear() && y !== year && (
                  <span className="ml-2 text-[9px] bg-gold-light/20 text-gold-dark font-bold px-1.5 py-0.5 rounded-full uppercase tracking-wide">
                    Actuelle
                  </span>
                )}
              </button>
