...
export default function GuidePelerinPage() {
  ...
  const duaColorsByIdx: Record<number, string> = {
    ...
    3: "bg-cream text-primary",
    4: "bg-cream text-primary",
    5: "bg-amber-100 text-gold-dark",
  };
          ...
          <div className="flex flex-wrap gap-3">
            {TOOLS.map((tool) => (
              <Link key={tool.href} href={tool.href as "/coran" | "/qibla" | "/guide-pelerin"}
                className="flex items-center gap-2.5 bg-gray-50 border border-gray-200 rounded-xl px-4 py-2.5 hover:border-primary-light hover:bg-cream transition-all group">
                <span className="text-primary group-hover:scale-110 transition-transform">{tool.icon}</span>
                <div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-[260px_1fr] gap-8">
            ...
            <aside className="hidden lg:block">
              <div className="sticky top-[140px] bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
                ...
                <div className="mt-6 bg-brand-deep rounded-xl p-4 text-white">
            ...
            <div className="space-y-10">
              ...
              <section id="intro" className="scroll-mt-28 bg-white rounded-2xl border border-gray-100 shadow-sm p-6 md:p-8">
                <div className="flex items-center gap-3 mb-5">
                  <div className="w-9 h-9 rounded-full bg-primary flex items-center justify-center text-white text-sm font-black flex-shrink-0">1</div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                  {pilgrimages.map((p, i) => (
                    <div key={p.title} className={`rounded-xl border p-4 ${
                      i !== 1 ? "bg-cream border-cream-dark" : "bg-amber-50 border-amber-200"
                    }`}>
                      {p.badge && (
                        <span className="inline-block text-xs font-bold bg-primary text-white px-2 py-0.5 rounded-full mb-2">{p.badge}</span>
                      )}
                      <p className="font-bold text-gray-900 text-sm">{p.title}</p>
                      <p className={`text-xs mb-2 font-arabic ${i !== 1 ? "text-primary" : "text-gold-dark"}`}>{p.arabic}</p>
              ...
              <section id="oumra" className="scroll-mt-28 bg-white rounded-2xl border border-gray-100 shadow-sm p-6 md:p-8">
                <div className="flex items-center gap-3 mb-5">
                  <div className="w-9 h-9 rounded-full bg-primary flex items-center justify-center text-white text-sm font-black flex-shrink-0">2</div>
                ...
                <div className="bg-cream border border-cream rounded-xl p-4">
                  <p className="text-sm text-brand-deep font-semibold mb-2">{t("oumraActsTitle")}</p>
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                    {(t.raw("oumraActs") as string[]).map((act, i) => (
                      <div key={act} className="bg-white rounded-lg p-3 text-center border border-cream">
                        <div className="w-7 h-7 rounded-full bg-primary text-white text-xs font-black flex items-center justify-center mx-auto mb-1.5">{i + 1}</div>
              ...
              <section id="rites-oumra" className="scroll-mt-28 bg-white rounded-2xl border border-gray-100 shadow-sm p-6 md:p-8">
                <div className="flex items-center gap-3 mb-6">
                  <div className="w-9 h-9 rounded-full bg-primary flex items-center justify-center text-white text-sm font-black flex-shrink-0">3</div>
                  ...
                  <div className="space-y-6">
                    {oumraRituals.map((r) => (
                      <div key={r.step} className="flex gap-5">
                        <div className="w-9 h-9 rounded-full bg-primary flex items-center justify-center text-white text-xs font-black flex-shrink-0 z-10 shadow-sm">
                          {r.step}
                        </div>
              ...
              <section id="hadj" className="scroll-mt-28 bg-white rounded-2xl border border-gray-100 shadow-sm p-6 md:p-8">
                <div className="flex items-center gap-3 mb-5">
                  <div className="w-9 h-9 rounded-full bg-gold flex items-center justify-center text-white text-sm font-black flex-shrink-0">4</div>
                ...
                <div className="bg-amber-50 border border-amber-100 rounded-xl p-4 mb-4 text-center">
                  ...
                  <p className="text-gold-dark text-xs italic">{t("hajjVerseTr")}</p>
                </div>
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                  {hajjSchedule.map((d) => (
              ...
              <section id="rites-hadj" className="scroll-mt-28 bg-white rounded-2xl border border-gray-100 shadow-sm p-6 md:p-8">
                <div className="flex items-center gap-3 mb-6">
                  <div className="w-9 h-9 rounded-full bg-gold flex items-center justify-center text-white text-sm font-black flex-shrink-0">5</div>
                  ...
                  {hajjDays.map((day, i) => (
                    <div key={day.day} className={`rounded-xl border overflow-hidden ${i < 4 ? "border-amber-100" : "border-gray-100"}`}>
                      <div className={`px-5 py-3 flex items-center gap-3 ${i < 4 ? "bg-amber-50" : "bg-gray-50"}`}>
                        <div className={`w-8 h-8 rounded-full flex items-center justify-center text-white text-xs font-black flex-shrink-0 ${i < 4 ? "bg-gold" : "bg-gray-500"}`}>
                          {i + 1}
                        </div>
                        <div>
                          <p className={`text-xs font-bold tracking-wide ${i < 4 ? "text-gold" : "text-gray-500"}`}>{day.day}</p>
              ...
              <section id="duas" className="scroll-mt-28">
                <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 md:p-8 mb-4">
                  <div className="flex items-center gap-3 mb-2">
                    <div className="w-9 h-9 rounded-full bg-primary flex items-center justify-center text-white text-sm font-black flex-shrink-0">6</div>
                    ...
                    {filteredDuas.map((dua) => (
                      <div key={dua.id}
                        className={`rounded-xl border overflow-hidden transition-all ${
                          dua.highlight ? "border-cream-dark bg-cream/50" : "border-gray-100"
                        }`}>
                        <button
                        ...
                        >
                          ...
                          <span className={`ml-3 flex-shrink-0 w-6 h-6 rounded-full flex items-center justify-center transition-all duration-200 ${
                            expandedDua === dua.id ? "bg-primary text-white rotate-45" : "bg-gray-100 text-gray-500"
                          }`}>
                        ...
                        {expandedDua === dua.id && (
                          <div className="px-5 pb-5 border-t border-gray-100 pt-4 space-y-4">
                            {/* Arabic */}
                            <div className="bg-brand-deep rounded-xl p-4 text-right">
                            ...
                            <div className="bg-amber-50 border border-amber-100 rounded-xl p-3">
                              <p className="text-xs font-bold text-gold uppercase tracking-widest mb-1">{t("phonetic")}</p>
              ...
              {/* 7 — Conclusion */}
              <section id="conclusion" className="scroll-mt-28 bg-brand-deep rounded-2xl p-6 md:p-8 text-white">
                <div className="flex items-center gap-3 mb-5">
