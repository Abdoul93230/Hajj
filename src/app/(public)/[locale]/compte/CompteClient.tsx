  ...
  async function handleLogin(e: React.FormEvent) {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        // pèlerin → portail ; autres rôles → accueil
        router.push(data.role === "PILGRIM" ? "/compte/mon-dossier" : "/");
        router.refresh();
      }
    } finally {
      setLoading(false);
    ...
    <div className="py-16 bg-gray-50 min-h-[60vh]">
      <div className="max-w-md mx-auto px-4">
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
          ...
          {forgotMode ? (
            <ForgotPassword
              onBack={() => { setForgotMode(false); setTab("login"); }}
            />
          ) : (
            <div className="p-6">
              {error && (
                <div className="bg-amber-50 border border-amber-200 text-gold-dark text-sm rounded-lg px-4 py-2.5 mb-4">
                  {error}
                </div>
              )}
